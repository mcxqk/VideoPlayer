...
public final class MCNGDebugScreen extends Screen implements GraphEditorHost {
	...
	private static final GraphEditorI18n MINECRAFT_I18N = (key, fallback, args) -> {
		...
		return translated.equals(key) ? GraphEditorI18n.formatFallback(fallback, key, args) : translated;
	};
	...
	public boolean charTyped(CharacterEvent input) {
		if (input.isAllowedChatCharacter()) {
			...
			if (text.length() == 1 && charTyped(text.charAt(0), 0)) {
				return true;
			}
		}
	...
	private void renderOverlay(GuiGraphicsExtractor context) {
		...
		context.text(font, title, x + 8, y + 8, theme.primaryTextColor(), false);
		for (int index = 0; index < help.size(); index++) {
			context.text(font, help.get(index), x + 8, y + 24 + (index * 12), theme.secondaryTextColor(), false);
		}
		...
		context.text(font, translate("mcng.ui.debug.status", "Status: %s", statusMessage), x + 8, statusY + 4, theme.accentColor(), false);
		...
		context.text(font, translate("mcng.ui.debug.panel_title", "Debug"), layout.x() + 8, layout.y() + 8, theme.primaryTextColor(), false);
		context.text(font, session.isExecutionRunning() ? translate("mcng.ui.debug.running", "Running") : translate("mcng.ui.debug.idle", "Idle"), layout.x() + layout.width() - 46, layout.y() + 8, session.isExecutionRunning() ? theme.executionColor() : theme.secondaryTextColor(), false);

		List<String> debug = session.debugMessages();
		for (int index = 0; index < Math.min(debug.size(), 2); index++) {
			context.text(font, debug.get(debug.size() - 1 - index), layout.x() + 8, layout.y() + 24 + (index * 12), theme.secondaryTextColor(), false);
		}

		List<GraphError> errors = session.lastErrors();
		for (int index = 0; index < Math.min(errors.size(), 2); index++) {
			context.text(font, GraphEditorTranslations.formatError(i18n(), errors.get(index)), layout.x() + 8, layout.y() + 50 + (index * 10), theme.errorColor(), false);
		}

		context.text(font, translate("mcng.ui.debug.section.editor", "Editor"), layout.x() + DEBUG_PANEL_PADDING, settingsTitleY(layout), theme.primaryTextColor(), false);
		...
		for (DebugButton button : debugButtons(layout)) {
			...
			context.text(font, button.label(), button.x() + 6, button.y() + 5, theme.primaryTextColor(), false);
		}

		context.text(font, translate("mcng.ui.debug.section.variables", "Variables"), layout.x() + DEBUG_PANEL_PADDING, variablesTitleY(layout), theme.primaryTextColor(), false);
		for (VariableRow row : variableRows(layout)) {
			...
			context.text(font, row.label(), row.x() + 6, row.y() + 5, theme.secondaryTextColor(), false);
		}
	}
