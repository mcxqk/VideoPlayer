...
public final class GraphEditorComponent {
	...
	private void renderTopBar(GuiGraphicsExtractor context) {
	...
	private void renderBreadcrumbs(GuiGraphicsExtractor context, GraphEditorTheme theme) {
		...
		for (int index = 0; index < breadcrumbs.size(); index++) {
			...
			context.text(textRenderer, breadcrumb.label(), x, y, theme.accentColor(), false);
			x += textRenderer.width(breadcrumb.label());
			if (index < breadcrumbs.size() - 1) {
				context.text(textRenderer, " / ", x, y, theme.secondaryTextColor(), false);
