...
final class ImagePreviewNodeBodyComponent implements NodeBodyComponent {
	...
	private void renderPreviewArea(NodeBodyRenderContext context, NodeWidget.Bounds bounds) {
		GuiGraphicsExtractor drawContext = context.drawContext();
		...
		if (textureId != null && imageWidth > 0 && imageHeight > 0) {
			...
			drawContext.text(context.textRenderer(), info, infoX, infoY, theme.primaryTextColor(), false);
			return;
		}
	...
	private void renderPathField(NodeBodyRenderContext context, NodeWidget.Bounds bounds, String filePath) {
		...
		context.drawContext().text(context.textRenderer(), display, bounds.x() + GraphTextInputRenderer.CONTENT_PADDING_X, baselineY, color, false);
	}
	...
	private void renderButton(NodeBodyRenderContext context, NodeWidget.Bounds bounds, String label, int accentColor, boolean enabled) {
		...
		context.drawContext().text(context.textRenderer(), label, textX, textY, textColor, false);
	}

	private void drawTexture(GuiGraphicsExtractor context, NodeWidget.Bounds bounds) {
	...
	private void drawCenteredLabel(GuiGraphicsExtractor context, Font textRenderer, NodeWidget.Bounds bounds, String label, int color) {
