...
final class NodeWidgetRenderer {
	...
	static void render(
		...
		if (!compactReroute && widget.showHeaderTitle() && detailLevel != NodeRenderDetailLevel.MINIMAL) {
			...
			context.text(textRenderer, GraphEditorTranslations.nodeTitle(widget.i18n(), widget.nodeType()), widget.x() + widget.edgePadding(), titleY, theme.primaryTextColor(), false);
		}
	...
	private static void renderInputRow(
		...
		context.text(textRenderer, label, labelX, labelY, theme.primaryTextColor(), false);
		...
		context.text(textRenderer, trimText(textRenderer, renderedValue, row.fieldBounds().width() - 8), row.fieldBounds().x() + 4, row.fieldBounds().y() + 3, theme.secondaryTextColor(), false);
	}

	private static void renderOutputRow(GuiGraphicsExtractor context, Font textRenderer, NodeWidget widget, NodeWidget.OutputPortRowWidget row, GraphEditorTheme theme) {
	...
	private static void renderTextControlRow(
		...
		if (row.labelVisible()) {
			context.text(textRenderer, trimText(textRenderer, GraphEditorTranslations.controlLabel(widget.i18n(), widget.nodeType(), row.control()), labelWidthForControlRow(widget, row.fieldBounds())), widget.x() + widget.edgePadding(), row.y() + Math.max(2, (row.height() - textRenderer.lineHeight) / 2), theme.primaryTextColor(), false);
		}
		...
		context.text(textRenderer, trimText(textRenderer, value, row.fieldBounds().width() - 8), row.fieldBounds().x() + 4, row.fieldBounds().y() + 3, theme.secondaryTextColor(), false);
	}
	...
	private static void renderBooleanControlRow(
		...
		context.text(textRenderer, trimText(textRenderer, GraphEditorTranslations.controlLabel(widget.i18n(), widget.nodeType(), row.control()), labelWidthForControlRow(widget, row.toggleBounds())), widget.x() + widget.edgePadding(), row.y() + Math.max(2, (row.height() - textRenderer.lineHeight) / 2), theme.primaryTextColor(), false);
		...
		context.text(textRenderer, trimText(textRenderer, label, row.toggleBounds().width() - 8), row.toggleBounds().x() + 4, row.toggleBounds().y() + 3, theme.secondaryTextColor(), false);
	}
	...
	private static void renderCycleControlRow(
		...
		context.text(textRenderer, trimText(textRenderer, GraphEditorTranslations.controlLabel(widget.i18n(), widget.nodeType(), row.control()), labelWidthForControlRow(widget, row.valueBounds())), widget.x() + widget.edgePadding(), row.y() + Math.max(2, (row.height() - textRenderer.lineHeight) / 2), theme.primaryTextColor(), false);
		...
		context.text(textRenderer, "<", row.leftArrowBounds().x() + 4, row.leftArrowBounds().y() + 3, theme.secondaryTextColor(), false);
		context.text(textRenderer, ">", row.rightArrowBounds().x() + 4, row.rightArrowBounds().y() + 3, theme.secondaryTextColor(), false);
		...
		String currentLabel = row.control().options().stream()
			...
			.findFirst()
			.orElse(currentId);
		context.text(textRenderer, trimText(textRenderer, currentLabel, row.valueBounds().width() - 8), row.valueBounds().x() + 4, row.valueBounds().y() + 3, theme.secondaryTextColor(), false);
	}
