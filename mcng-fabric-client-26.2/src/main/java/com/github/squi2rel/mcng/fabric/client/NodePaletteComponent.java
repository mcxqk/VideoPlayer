...
public final class NodePaletteComponent {
	...
	public void render(GuiGraphicsExtractor context, Font textRenderer, int mouseX, int mouseY, float delta) {
		...
		context.text(textRenderer, GraphEditorTranslations.ui(i18n, "palette.title", "Nodes"), panelX() + PANEL_PADDING, panelY() + 10, theme.primaryTextColor(), false);
			...
			for (Row row : rows) {
				if (row.type() == RowType.SECTION) {
					if (y + SECTION_HEADER_HEIGHT >= listTop && y <= listBottom) {
						context.text(textRenderer, row.title(), panelX() + PANEL_PADDING, y + 4, theme.accentColor(), false);
					}
				...
				if (y + ENTRY_HEIGHT >= listTop && y <= listBottom) {
					...
					context.text(textRenderer, row.entry().displayName(), panelX() + PANEL_PADDING + 6, y + 5, theme.primaryTextColor(), false);
					context.text(textRenderer, row.entry().subtitle(), panelX() + PANEL_PADDING + 6, y + 14, theme.secondaryTextColor(), false);
				}
				y += ENTRY_HEIGHT;
			}
		} finally {
	...
	public void renderDragPreview(GuiGraphicsExtractor context, Font textRenderer) {
		if (!state.dragging() || state.dragEntry() == null) {
			return;
		}
		GraphEditorUiConfig uiConfig = uiConfigSupplier.get();
	...
	private void renderToggleButton(GuiGraphicsExtractor context, Font textRenderer, GraphEditorUiConfig uiConfig, GraphEditorTheme theme) {
	...
	private void renderScrollbar(GuiGraphicsExtractor context, List<Row> rows, int listTop, int listBottom, GraphEditorUiConfig uiConfig, GraphEditorTheme theme) {
